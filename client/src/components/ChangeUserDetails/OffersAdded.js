...
const OffersAdded = ({ username }) => {
    const [offers, setOffers] = useState([]);

    useEffect(() => {
        const fetchOffers = async () => {
            const response = await UserService.getOffersAdded(username);
                ...
                {Array.isArray(offers) && offers.length > 0 ? (
                    offers.map((offer) => (
                        <div key={offer.id} className={`offer-card`}>
                            ...
                            <div className={`likes-dislikes out-of-stock-gray`}>
